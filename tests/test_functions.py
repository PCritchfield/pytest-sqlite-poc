...
from src.database.functions import calculate_postage, validate_address, generate_tracking, register_functions
...
def test_register_functions(db_connection):
    ...
    result = execute_query(
        db_connection,
        "SELECT calculate_postage(1.5, 3) AS postage"
    )
...
def test_calculate_postage_function(db_connection):
    ...
    for weight, zone, expected in test_cases:
        result = execute_query(
            db_connection,
            "SELECT calculate_postage(?, ?) AS postage",
            (weight, zone)
        )
        
        # Allow for small floating-point differences
        assert abs(result[0]["postage"] - expected) < 0.001, \
...
def test_validate_address_function(db_connection):
    ...
    test_cases = [
        # Valid address
        ('{"street_line1": "123 Main St", "city": "Anytown", "state": "OH", "postal_code": "12345"}', 1),
        # Missing street
        ('{"city": "Anytown", "state": "OH", "postal_code": "12345"}', 0),
        # Missing city
        ('{"street_line1": "123 Main St", "state": "OH", "postal_code": "12345"}', 0),
        # Invalid postal code format
        ('{"street_line1": "123 Main St", "city": "Anytown", "state": "OH", "postal_code": "1234"}', 0),
        # Valid with ZIP+4
        ('{"street_line1": "123 Main St", "city": "Anytown", "state": "OH", "postal_code": "12345-6789"}', 1),
    ]
    
    for address_json, expected in test_cases:
        result = execute_query(
            db_connection,
            "SELECT validate_address(?) AS is_valid",
            (address_json,)
        )
        
        assert result[0]["is_valid"] == expected, \
...
def test_generate_tracking_function(db_connection):
    ...
    for carrier in carriers:
        result = execute_query(
            db_connection,
            "SELECT generate_tracking(?) AS tracking",
            (carrier,)
        )
...
def test_batch_counter_aggregate(db_connection):
    ...
    result = execute_query(
        db_connection,
        "SELECT batch_count(value) AS count FROM test_batch"
    )
