...
from src.database.connection import get_connection, execute_script
from src.database.schema import create_tables, drop_tables
from src.database.functions import register_functions, create_triggers
...
def insert_sample_data(conn: sqlite3.Connection) -> None:
    ...
    tables = [
        "delivery_tracking", "print_queue", "mail_items", "print_jobs",
        "mailing_campaigns", "list_members", "mailing_lists", "inventory",
        "materials", "addresses", "customers"
    ]
    ...
    for customer_id in customer_ids:
        ...
        addresses.append((
            None, customer_id, "home", f"{customer_id*123} Main St", None, 
            "Anytown", "OH", f"{customer_id+10000}", "USA", 1
        ))
        
        # Work address for some customers
        if customer_id % 2 == 0:
            addresses.append((
                None, customer_id, "work", f"{customer_id*100} Business Ave", f"Suite {customer_id*10}", 
                "Workville", "OH", f"{customer_id+20000}", "USA", 1
            ))
    ...
    for list_id in list_ids:
        for i, (address_id, customer_id) in enumerate(address_data):
            ...
            if i % len(list_ids) == list_id % len(list_ids):
                list_members.append((
                    None, list_id, customer_id, address_id, "active"
                ))
    ...
    # Insert print jobs
    print_jobs = [
        (None, "January Newsletter Batch", "First batch of January newsletter", "queued", "2025-01-10", None, None),
        (None, "Winter Sale Preview", "Preview batch for winter sale", "queued", "2025-01-12", None, None)
    ]
    conn.executemany(
...
def create_sample_data(db_path):
    ...
    Args:
        db_path: Path to the SQLite database file
    """
    # This now just calls the generate_sample_data function from sample_data.py
